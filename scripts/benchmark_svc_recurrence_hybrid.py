"""Benchmark the fixed SVC–Recurrence hybrid against both source strategies."""
...
def _summary(
    ...
    return {
        ...
        "liftOverTheoreticalRandom": (
            sum(hits) - len(hits) * 36 / 49
        ),
    }
...
def benchmark(dataset_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "promotion": {
            "passed": False,
            "rule": (
                "Remain experimental because the hybrid must not trail both "
                "sources on validation before holdout improvement can promote it."
            ),
        },
    }
