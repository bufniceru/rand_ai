...
onMounted(async () => {
  document.title = "Last Seen Gap Highlight — Rand AI";
  if (!window.randAiDesktop) {
    errorMessage.value =
      "Last Seen Gap Highlight is available inside the Electron desktop application.";
    return;
  }
...
<template>
  <main class="last-seen-dialog-shell">
    ...
    <section v-else class="dialog-empty-state">
      <strong>Last Seen Gap Highlight unavailable</strong>
